'''
metrics source: https://gist.github.com/ilmonteux/8340df952722f3a1030a7d937e701b5a
'''
...
def mean_dice_np(y_true, y_pred, **kwargs):
  ...
  # keras version
def seg_metrics(y_true, y_pred, metric_name, metric_type='standard', drop_last = True, mean_per_class=False, verbose=False, class_id=None):
    """ 
    ...
    if class_id:
        metric = K.reshape(metric[:,class_id],(-1, 1))
        mask = K.reshape(mask[:,class_id], (-1, 1))
    ...
    return K.mean(non_zero_sum / (non_zero_count+1e-7))
...
def background_mean_iou(y_true, y_pred):
    return seg_metrics(y_true, y_pred, metric_name='iou', drop_last=False, class_id=2)

def negative_mean_iou(y_true, y_pred):
    return seg_metrics(y_true, y_pred, metric_name='iou', drop_last=False, class_id=0)

def positive_mean_iou(y_true, y_pred):
    return seg_metrics(y_true, y_pred, metric_name='iou', drop_last=False, class_id=1)   